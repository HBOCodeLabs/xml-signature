...
SignedXml.prototype.createSignedInfo = function(doc, c14n, signatureAlgorithm, referenceNode) {
  var res =
    '<SignedInfo>' +
    '<CanonicalizationMethod Algorithm="' + c14n.getAlgorithmName() + '" />' +
    '<SignatureMethod Algorithm="' + signatureAlgorithm.getAlgorithmName() + '" />' +
    referenceNode +
    '</SignedInfo>';
...
SignedXml.prototype.createReferenceNode = function(doc, nodeToSign, takeUriFromIdInRootElement, transforms, digestAlgorithm) {
  ...
  if (takeUriFromIdInRootElement) {
    res += '<Reference URI="#' + this.ensureHasId(node) + '">';
  } else {
    res += '<Reference URI="">';
  }
  ...
  res +=  '</Transforms>'+
          ...
          '<DigestValue>' + digestAlgorithm.getHash(transformedCanonXml) + '</DigestValue>'+
          '</Reference>';
